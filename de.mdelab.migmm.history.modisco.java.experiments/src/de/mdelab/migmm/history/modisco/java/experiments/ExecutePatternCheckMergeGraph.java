...
public class ExecutePatternCheckMergeGraph {
		...
		for(int index1 = 0; index1 < model.getOwnedTimingUnits().size() - 1; index1++) {
			...
			for(int index2 = index1 + 1; index2 < model.getOwnedTimingUnits().size(); index2++) {
				...
				for(TimingUnit c:commonPredecessors.get(i).get(j)) {
					Collection<EObject> mergeResult = merger.computeMerge(presentSets.get(i), presentSets.get(j), presentSets.get(c));
					...
					Collection<EObject> roots = findRoots(pattern, mergeResult);
